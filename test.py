...
def preprocessing(img):
    # claher = cv2.createCLAHE(clipLimit=3, tileGridSize=(5, 5))
    # claheg = cv2.createCLAHE(clipLimit=2, tileGridSize=(5, 5))
    # claheb = cv2.createCLAHE(clipLimit=1, tileGridSize=(5, 5))
    ...
    # blended = cv2.addWeighted(rgb_img, 0.6, center_crop, 0.4, 0)

    # # 双边滤波
    # center_crop = cv2.bilateralFilter(blended, d=5, sigmaColor=15, sigmaSpace=15)
    # # center_crop=blended
    # 锐化图像
    ...
    # claher = cv2.createCLAHE(clipLimit=3, tileGridSize=(4,4))
    # claheg = cv2.createCLAHE(clipLimit=2, tileGridSize=(4,4))
    # claheb = cv2.createCLAHE(clipLimit=1, tileGridSize=(4,4))
    # cllr = claher.apply(imgr)
    # cllg = claheg.apply(imgg)
    # cllb = claheb.a-pply(imgb)

    # rgb_img = np.dstack((cllr, cllg, cllb))

    # # 锐化图像
    # kernel_sharpen_1 = np.array([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]])

    # # center_crop = cv2.filter2D(blurred, -1, kernel_sharpen_1)
    # center_crop = cv2.filter2D(rgb_img, -1, kernel_sharpen_1)
