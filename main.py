...
print("此環境Torch版本:",torch.__version__)
print("Torch對應的Cuda版本:",torch.version.cuda)
print("此環境Cuda版本:",torch.version.cuda)
print("此環境Torch和Cuda版本是否對應:",torch.cuda.is_available())
print("此環境Cudnn版本:",torch.backends.cudnn.version())
print("查看此環境cuda數目:",torch.cuda.device_count())
...
folder='C:\\Users\\user\\Desktop\\AICup\\part1\\val\\'
...
for i in os.listdir(folder):
    ...
    for j in os.listdir(tmppath):
        ...
        video_file = os.path.join(tmppath,j)
        ...
        while success:
            ...
            # Inference
            with torch.no_grad():
                y_pred = model(x.cuda())#(x.cpu())
            y_pred = y_pred.detach().cpu().numpy()#.numpy()
            h_pred = y_pred > 0.5
            h_pred = h_pred * 255.0
        ...
        while cap.isOpened():
            ...
            if frame_count in final_predict:  # 每两帧图像才处理一次
                ...
                top = int((h - center_crop_size) / 2)#360
                ...
                resized_width, resized_height = 1280,1280
                ...
                if datum==None :#or datum.poseKeypoints==None:
                    continue
                ...
                for i in range(len(people)):
                    ...
                    if people[i][6]==hitter:
                        ...
                        locationX=int(scale_x *people[i][0]) + left
                        locationY= int(people[i][1]* scale_y) + top + padding
                        ...
                        defendX=int(scale_x *people[i][0]) + left
                        defendY=int(people[i][1]* scale_y) + top + padding
                print(f'locX:{locationX},locY:{locationY},defX:{defendX},defY:{defendY}')
                import random
                if frame_count==final_predict[-1]:
                    courtdf=court_superglue.court_superglue(frame.copy())
                    winner=winner_detection.caculatewinner(courtdf,(int(ball_x),int(ball_y)))
                    finaldf=finaldf.append(pd.Series([video_name,count,frame_count,hitter,2,backhand,random.randint(1,2),int(ball_x),int(ball_y),int(locationX),int(locationY),int(defendX),int(defendY),random.randint(1, 9),winner],index=finaldf.columns), ignore_index=True)
                else:
                    finaldf=finaldf.append(pd.Series([video_name,count,frame_count,hitter,2,backhand,random.randint(1,2),int(ball_x),int(ball_y),int(locationX),int(locationY),int(defendX),int(defendY),random.randint(1, 9),'X'],index=finaldf.columns), ignore_index=True)
                count+=1
                ...
                torch.cuda.empty_cache()  # 釋放 GPU 內存
        #end while   
        #if all frame out,but still no result,we can do it below
        if finaldf.loc[finaldf.index[-1], 'Winner'] == 'X':
            courtdf = court_superglue.court_superglue(clahe(allframe[-1].copy()))
            winner = winner_detection.caculatewinner(courtdf, (int(finaldf.loc[finaldf.index[-1], 'LandingX']), int(finaldf.loc[finaldf.index[-1], 'LandingY'])))
            finaldf.loc[finaldf.index[-1], 'Winner'] = winner
            # print(winner)
